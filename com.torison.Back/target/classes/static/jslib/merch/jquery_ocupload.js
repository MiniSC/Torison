 ...
 *
 * 由VAL修改，加入了height和width设置，用于解决特殊情况下无法获取对象的问题
 * $.ocupload($(element),{height:20,width:200}); 强制设置对象大小
 */
(function($){
	...
	$.ocupload = function(element, options) {
		...
		var container = element.parent().css({
			position: 'relative',
			height: (options.height?options.height:element.outerHeight())+'px',
			width: (options.width?options.width:element.outerWidth())+'px',
			overflow: 'hidden',
			cursor: 'pointer',
			margin: 0,
			padding: 0,
