...
function payDialog(settleId){
	$("#ModalLabel").text("结算单确认打款");
	var html = "<p>结算单打款：</p>";
	html = html + settleOrderDetail(settleId);
	html = html + "<input type='hidden' name='id' value='" + settleId + "'>";
	html = html + "<input type='hidden' name='payStatus' value='1'>";
	html = html + "<p>打款银行："+$("#bankListSelectId").html()+"<span id='bankNameError' class='errorNote'></span></p>";
	html = html + "<p>打款账户：<input type='text' readOnly='readOnly' name='payAccount' maxlength='20' id='bankAccountInput' class='input-large'/><span id='bankAccountError' class='errorNote'></span></p>";
	html = html + "<p>打款日期：<input id='payDate' readOnly='readOnly' name='payDate' class='input-medium Wdate' type='text'>";
	html = html + "<p><span class='sep_20'></span><span class='sep_20'></span>";
	html = html + "<button type='button' class='btn btn-primary' id='payConfirm'>确定</button>";
	html = html + "<span class='sep_20'></span>";
	html = html + "<button type='button' class='btn' data-dismiss='modal'>取消</button></p>";
	$("#ModelContent").html(html);
	$("#ModelBtnBox").hide();
	$("#modelForm").attr("action",payActionUrl);
	$('#myModal').modal('show');
	init_My97_one('payDate');
	$("#payConfirm").bind("click",process);
	$("#modelCancel").one("click",function (){
		$('#myModal').modal('hide');
		$('#modelConfirm').unbind("click",process);
	});
	function process(){
		$('#modelConfirm').unbind("click",process);
		var bankName = $.trim($("p #bankNameInput").val()); 
		...
		Ajax(payActionUrl,param,'打款成功','打款失败');
		return false;
	}
	return false;
}
